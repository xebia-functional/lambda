...
object EventsAApp extends RequestHandler[KinesisEvent, Unit]{
  ...
  def computeHash(d: Datum): Datum =
    if d.hash.isEmpty then
      ...
      d.copy(hash = Some(hashedBytes.foldLeft(new StringBuilder())((sb, b) => sb ++= String.format("%02x", Byte.box(b))).toString))
    else d
  ...
  override def handleRequest(event: KinesisEvent, context: Context): Unit =
    ...
    val prg = 
      for 
        ...
        stream <- IO.fromOption(streamOpt)(new RuntimeException(s"Missing environment variable $WRITE_STREAM"))
        hashedRecords <- event.getRecords.asScala.toList.traverse(processRecord(parser)) 
        kinesis <- Kinesis.kinesisClient
        successfullyPosted <- Kinesis.postData(kinesis, stream, hashedRecords.flatten)
        _ <- log.debug(s"Posted $successfullyPosted messages")
      yield ()
    prg.unsafeRunSync()

  def processRecord(parser: JawnParser)(record: KinesisEventRecord)(using log: Logger[IO]): IO[Option[Datum]] =
    for 
      _ <- log.trace(s"Incoming record: $record")
      json <- IO.pure(parser.parseByteBuffer(record.getKinesis.getData).toOption)
