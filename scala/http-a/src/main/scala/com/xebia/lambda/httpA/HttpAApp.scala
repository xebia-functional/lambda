...
object HttpAApp extends RequestHandler[APIGatewayV2HTTPEvent, APIGatewayV2HTTPResponse] {
  ...
  private def getParameterOrDefault(event: APIGatewayV2HTTPEvent, key: String, default: Int): Int =
    (for {
      param <- Option(event.getQueryStringParameters).flatMap(params => Option(params.get(key)))
      value <- Try(param.toInt).toOption
    } yield value).getOrElse(default)

  def handleRequestIO(kinesis: AmazonKinesisAsync, event: APIGatewayV2HTTPEvent)(using log: Logger[IO]) : IO[APIGatewayV2HTTPResponse] =
    ...
    for
      ...
      data <- List.fill(messages)((chars, hashes)).traverse(Datum.random[IO].tupled)
  ...
  def handleRequest(event: APIGatewayV2HTTPEvent, ctx: Context): APIGatewayV2HTTPResponse =
